-- terras_capixabas/veiculos/rural.lua
...
-- Generic Rural Entity Definition Function
local function create_rural_entity(name, texture, description, inventory_image)
    core.register_entity(name, {
        ...
        on_step = function(self, dtime)
            ...
            if self._acceleration ~= 0 then
                self._speed = self._speed + self._acceleration * 0.15
            else
            ...
            if math_abs(self._speed) > 0.1 then
                ...
                if not self._sound_handle then
                    self._sound_handle = core_sound_play("kombi", {object = self.object, loop = true, gain = 0.7, max_hear_distance = 16})
                end
                ...
                if not self._idle_sound_handle then
                    self._idle_sound_handle = core_sound_play("kombi_idle", {object = self.object, loop = true, gain = 0.5, max_hear_distance = 16})
                end
                if self._current_anim ~= "idle" then
                    self.object:set_animation({x=0, y=0.4}, 1, 0)
                    self._current_anim = "idle"
...
create_rural_entity("terras_capixabas:vh_rural", "rural.png", "Rural", "rural_inv.png")
create_rural_entity("terras_capixabas:vh_rural_verde", "rural_verde.png", "Rural Verde", "rural_verde_inv.png")
create_rural_entity("terras_capixabas:vh_rural_vermelha", "rural_vermelha.png", "Rural Vermelha", "rural_vermelha_inv.png")