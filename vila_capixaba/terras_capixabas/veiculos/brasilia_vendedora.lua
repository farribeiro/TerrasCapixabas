-- terras_capixabas/veiculos/brasilia_vendedora.lua
...
-- Brasília Vendedora Entity
core.register_entity("terras_capixabas:vh_brasilia_vendedora", {
    initial_properties = {
        visual = "mesh",
        mesh = "brasilia_vendedora.glb",
        textures = {"brasilia_vendedora.png"},
        visual_size = {x=1, y=1},
        physical = true,
    ...
    on_step = function(self, dtime)
        -- GRAVITY FIX: Moved outside the driver check so the car always falls
        local pos = self.object:get_pos()
        self.object:set_acceleration({x = 0, y = -9.81, z = 0})
        
        -- Ground check to stop falling
...
-- Brasília Vendedora Spawn Egg
core.register_craftitem("terras_capixabas:vh_brasilia_vendedora_spawn_egg", {
    description = "Brasília Vendedora",
    inventory_image = "brasilia_vendedora_inv.png",
    
    on_place = function(itemstack, placer, pointed_thing)
        if pointed_thing.type ~= "node" then return itemstack end
        local pos = pointed_thing.above
        -- SPAWN FIX: Changed from 0.5 to 0.1 to prevent air-stuck bug
        pos.y = pos.y + 0.1
        local obj = core.add_entity(pos, "terras_capixabas:vh_brasilia_vendedora")
