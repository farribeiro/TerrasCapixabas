-- terras_capixabas/veiculos/fusca.lua
...
-- Generic Fusca Entity Definition Function
local function create_fusca_entity(name, texture, description, inventory_image)
    core.register_entity(name, {
        ...
        on_rightclick = function(self, clicker)
            ...
            if not self._driver then
                ...
                clicker:set_attach(self.object, "", {x = 0, y = 7, z = 0}, {x = 0, y = 0, z = 0})
                ...
                self._idle_sound_handle = core_sound_play("fusca_idle", {
        ...
        on_step = function(self, dtime)
            ...
            if ctrl.left then yaw = yaw + math_rad(2) elseif ctrl.right then yaw = yaw - math_rad(2) end
            ...
            if math_abs(self._speed) > 0.1 then
                ...
                if not self._sound_handle then
                    self._sound_handle = core_sound_play("fusca", {object = self.object, loop = true, gain = 0.7, max_hear_distance = 16})
                end
                ...
                if not self._idle_sound_handle then
                    self._idle_sound_handle = core_sound_play("fusca_idle", {object = self.object, loop = true, gain = 0.5, max_hear_distance = 16})
                end
                if self._current_anim ~= "idle" then
                    self.object:set_animation({x=0, y=0.4}, 1, 0)
                    self._current_anim = "idle"
...
create_fusca_entity("terras_capixabas:vh_fusca_azul_celeste", "fusca_azul_celeste.png", "Fusca Azul Celeste", "fusca_azul_celeste_inv.png")
create_fusca_entity("terras_capixabas:vh_fusca_amarelo", "fusca_amarelo.png", "Fusca Amarelo", "fusca_amarelo_inv.png")
create_fusca_entity("terras_capixabas:vh_fusca_vermelho", "fusca_vermelho.png", "Fusca Vermelho", "fusca_vermelho_inv.png")