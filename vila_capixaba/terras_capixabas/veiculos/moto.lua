-- terras_capixabas/veiculos/moto.lua
...
local function create_moto_entity(name, texture, description, inventory_image)
...
collisionbox={-0.4,0.0,-0.8,0.4,1.2,0.8},
stepheight=0.5,
...
if not driver or not driver:is_player() then self._driver=nil self._is_driving=false return end
...
clicker:set_attach(self.object,"",{x=0,y=9,z=0},{x=0,y=0,z=0})
...
if self._idle_sound_handle then core_sound_stop(self._idle_sound_handle) self._idle_sound_handle=nil end
self._idle_sound_handle=core_sound_play("moto_idle",{object=self.object,loop=true,gain=0.5,max_hear_distance=16})
else
...
if vel.y<0 then self.object:set_velocity({x=vel.x,y=0,z=vel.z}) end
end
...
self.object:set_animation({x=0,y=0.25},1,0)
self._current_anim="idle"
end
if self._driver and not self._driver:is_player() then self._driver=nil self._is_driving=false end
return
end
...
if ctrl.jump then self:detach_driver() return end
...
self._speed=self._speed+self._acceleration*0.3
else
if self._speed>0 then
self._speed=self._speed-0.15
...
if ctrl.left then yaw=yaw+math_rad(3) elseif ctrl.right then yaw=yaw-math_rad(3) end
...
self._sound_handle=core_sound_play("moto",{object=self.object,loop=true,gain=0.7,max_hear_distance=16})
end
local anim=self._speed>0 and "forward" or "reverse"
if self._current_anim~=anim then
self.object:set_animation({x=1.0,y=1.50},(anim=="forward" and 2 or -2),0)
...
create_moto_entity("terras_capixabas:vh_moto","moto.png","Moto","moto_inv.png")
create_moto_entity("terras_capixabas:vh_moto_azul","moto_azul.png","Moto Azul","moto_azul_inv.png")
create_moto_entity("terras_capixabas:vh_moto_roxa","moto_roxa.png","Moto Roxa","moto_roxa_inv.png")
