...
local function get_hunger(player)
    return tonumber(player:get_meta():get_string("hunger")) or 0
end

local function get_thirst(player)
    return tonumber(player:get_meta():get_string("thirst")) or 0
end
...
core.register_craftitem("terras_capixabas:alm_"..name,{
description=desc,
inventory_image=name..".png",
on_secondary_use=function(itemstack,user,pointed_thing)
set_hunger(user,get_hunger(user)-1)
update_hud(user)
core.sound_play(sound,{to_player=user:get_player_name()})
itemstack:take_item()
return itemstack
end
})
...
-- Frozen pizza (not edible, used in microwave)
core.register_craftitem("terras_capixabas:pizza_congelada",{
description="Pizza Congelada",
inventory_image="pizza_congelada.png"
})