...
animations = {
idle  = {start=0,    stop=0.04, speed=1},
walk  = {start=0.25, stop=1,    speed=1},
sleep = {start=1.5,  stop=1.54, speed=1},
},
...
not self.following_player or
not self.following_player:is_player() or
self.following_player:get_wielded_item():get_name() ~= "terras_capixabas:alm_sacanagem"
) then
self.following = false
self.following_player = nil
self.state = "idle"
...
if self.sidewalk_dir == "forward" and pos.z <= -395 then self.sidewalk_dir = "backward"
elseif self.sidewalk_dir == "backward" and pos.z >= -225 then self.sidewalk_dir = "forward" end
